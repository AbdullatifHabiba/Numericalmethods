...
public class test {
    ...
    double[][] Coeff(String l){
        ...
        for (int i = 0;i < C.length / 2;i++){
            coeff[i][C.length / 2] = F.precision(Double.parseDouble(C[2 * i + 1]));
        }
        for (int i = 0;i < C.length / 2;i++){
                ...
                if (isLetter(C[2 * i].charAt(k))){
                    if (k == 0)
                        ...
                        if (j == 0 && !isLetter(C[2 * i].charAt(0)))
                            coeff[i][unkowns.indexOf(C[2 * i].charAt(k))] = F.precision(Double.parseDouble(C[2 * i].substring(0,k)));
                        else
                            coeff[i][unkowns.indexOf(C[2 * i].charAt(k))] = F.precision(Double.parseDouble(C[2 * i].substring(j + 1,k)));
                    }
                }
            }
        }
